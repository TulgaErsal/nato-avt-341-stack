...
/// RViz tool for setting an avt_341_msgs/NavGoal by clicking to place the goal
/// and dragging to orient it -- exactly like the built-in "2D Goal Pose" tool.
///
...
protected:
    /// Build and publish a NavGoal from the placed pose plus the threshold
    /// properties. \p x, \p y are in the fixed frame and \p theta is the yaw.
    void onPoseSet( double x, double y, double theta ) override;

private Q_SLOTS:
    /// (Re)create the publisher when the topic name or QoS settings change.
    void updateTopic();

private:
    rclcpp::Publisher<avt_341_msgs::msg::NavGoal>::SharedPtr publisher_;
