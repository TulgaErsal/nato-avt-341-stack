...
void MessageCallback(avt_341_msgs::msg::Communication::SharedPtr msg,
                     const bool verbose_comm_log) {
    pending_msgs.push(*msg);
    if(verbose_comm_log){
      RCLCPP_INFO(nh->get_logger(), "Received sender=%s,msg_id=%d,type=%s,receiver=%s to broadcast.", msg->sender_name.c_str(), msg->msg_id, msg->type.c_str(), msg->receiver_name.c_str());
    }
}
...
int main(int argc, char* argv[])
{
    ...
    auto msg_sub = nh->create_subscription<avt_341_msgs::msg::Communication>(
        "avt_341/comm_messages", 10,
        [&params](avt_341_msgs::msg::Communication::SharedPtr msg) {
            MessageCallback(msg, params.verbose_comm_log);
        });

    // Set up publishers
    auto msg_pub = nh->create_publisher<avt_341_msgs::msg::Communication>("avt_341/comm_messages", 10);

    int msg_count = 0;
    char buffer[256];
    avt_341_msgs::msg::Communication packed_msg;
    ...
    std::vector<std::shared_ptr<rclcpp::Publisher<avt_341_msgs::msg::Communication>>> other_veh_pubs;
    if(params.broadcast_over_ros){
      for(const auto & veh_ns: params.vehicle_namespaces){
        ...
        if(veh_ns_upper != params.name){
          other_veh_pubs.push_back(nh->create_publisher<avt_341_msgs::msg::Communication>("/" + veh_ns + "/avt_341/comm_messages", 10));
        }
      }
    }
    ...
    if(!client->connect())
        RCLCPP_INFO(nh->get_logger(), "Error connecting to the server\n");
    ...
    while (rclcpp::ok()) {
        ...
        while(!pending_msgs.empty()) {
            ...
            avt_341_msgs::msg::Communication next_msg = pending_msgs.front();
            pending_msgs.pop();
            ...
            if(params.verbose_comm_log){
              RCLCPP_INFO(nh->get_logger(), "Broadcasting %s", msg_serialized.c_str());
            }
            ...
            if(params.broadcast_over_ros){
              ...
              if(n < 0)
                RCLCPP_ERROR(nh->get_logger(), "Error writing to socket\n");
              else {
                RCLCPP_DEBUG(nh->get_logger(), "Message sent and buffer cleared.\n");
              }
            }
        }
        ...
        if(client->read_available(buffer, 256) > 0)
        {
            ...
            if(params.verbose_comm_log){
              RCLCPP_INFO(nh->get_logger(), "Read %s", buffer_str.c_str());
            }
