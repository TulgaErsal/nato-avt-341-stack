...
void DeclareParameters() {
    ...
    // Declare the Julia module path parameter.
    node->get_parameter("~julia_module_path", module_path, std::string());
...
void InitialiseJuliaAPI() {
    ...
    if(!module_path.empty()) {
        node->log_info("Loading Julia module from user-defined path at: %s ...",
                       module_path);
    } else if(!strlen(MPC_PLANNER_PATH) == 0) {
        node->log_warning(
        ...
        node->log_error(
            ...
            "CMake build log for variable MPC_PLANNER_PATH or define the "
            "parameter ~julia_module_path manually.");
        has_error = EXIT_FAILURE;
        jl_atexit_hook(has_error);
        throw std::invalid_argument(
            "No valid path to the Julia MPC module could be found.");
    }
    std::string path(std::string("Base.include(Main, \"") + MPC_PLANNER_PATH +
                     std::string("\")"));

    node->log_info("Loading Julia MPC planner at: %s", MPC_PLANNER_PATH);
