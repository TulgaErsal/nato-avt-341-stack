...
SensorBridgeNode::SensorBridgeNode() : Node("sensor_bridge_node") {
    ...
    if(this->sensorType == "LIDAR" || this->sensorType == "Lidar" ||
       this->sensorType == "lidar") {
        this->lidarDataPublisher_ =
    ...
    if(this->sensorType == "RADAR" || this->sensorType == "Radar" ||
       this->sensorType == "radar") {
        this->radarDataPublisher_ =
            this->create_publisher<sensor_msgs::msg::PointCloud2>(
                this->rosTopic,
                qos);
        while(rclcpp::ok()) {
    ...
    if(this->sensorType == "CAMERA" || this->sensorType == "Camera" ||
       this->sensorType == "camera") {
        this->cameraImageType = 0;
        this->cameraDataPublisher_ =
            this->create_publisher<sensor_msgs::msg::Image>(this->rosTopic,
                                                            qos);
        std::unique_ptr<CameraDataDeserializerDefault> cameraDeserializer(
            new CameraDataDeserializerDefault);
        cameraDeserializer->Initialize();
        while(rclcpp::ok()) {
            try {
                SensorBridgeNode::publishCameraData(cameraDeserializer,
                                                    this->sensorId,
                                                    this->cameraImageType);
            } catch(const std::exception& e) {
    ...
    while(vesiSensorConnection == false) {
        try {
            ...
            if(retries < max_retries) {
                std::cout << "Failed for the " << retries
                          << ". time. Try again." << std::endl;
                retries++;
                rclcpp::sleep_for(5s);
            } else {
...
void SensorBridgeNode::publishLidarData(uint8_t sensorId) {
    ...
    if(this->verbosePrinting) {
        std::cout << "\tRequested lidar sensor with ID " << unsigned(sensorId)
                  << " at maneuver time: "
                  << sensorData.getSensorHeader().m_uiTimeStamp << "\xE6s"
                  << std::endl;
        for(size_t i = 0; i < lidarFrame.LidarPoints.size() &&
            i < this->MAX_SENSOR_RESULT_PRINTOUTS;
            i++) {
            const PointCloudLidarPoint& point = lidarFrame.LidarPoints[i];
            std::cout << "\t\tP[" << i << "] azimuth=" << point.Azimuth
    ...
    lidarData.point_step =
        7 * sizeof(float); // azimuth, elevation, depth, x, y, z, reflectivity
    ...
    for(size_t i = 0; i < lidarData.width; ++i,
               ++iter_x,
               ++iter_y,
               ++iter_z,
               ++iter_azi,
        ...
        *iter_x = static_cast<float>(point.Distance * cos(elevationRad) *
                                     cos(azimuthRad));
        *iter_y = static_cast<float>(point.Distance * cos(elevationRad) *
                                     sin(azimuthRad) * (-1));
...
void SensorBridgeNode::publishRadarData(uint8_t sensorId) {
    ...
    // Print some data points
    if(this->verbosePrinting) {
        std::cout << "\tRequested radar sensor with ID " << unsigned(sensorId)
                  << " at maneuver time: "
                  << sensorData.getSensorHeader().m_uiTimeStamp << "\xE6s"
                  << std::endl;
        for(size_t i = 0;
            i < detections.size() && i < this->MAX_SENSOR_RESULT_PRINTOUTS;
            i++) {
            const Detection& detection = detections[i];
            std::cout << "\t\tDetection [" << i
                      << "]: azimuth=" << detection.AzimuthAngle
                      << " elevation=" << detection.ElevationAngle
                      << " radial_distance=" << detection.RadialDistance
                      << " radial_velocity=" << detection.RadialVelocity
                      << std::endl;
        }
    }
    ...
    radarData.point_step = 10 *
        sizeof(float); // azimuth, elevation, depth, x, y, z, reflectivity, x,
                       // y, z, radial velocity, existence probability, SNR, RCS
    ...
    sensor_msgs::PointCloud2Iterator<float> iter_radVel(radarData,
                                                        "radial_velocity");
    sensor_msgs::PointCloud2Iterator<float> iter_RCS(radarData,
                                                     "radar_cross_section");
    sensor_msgs::PointCloud2Iterator<float> iter_SNR(radarData,
                                                     "signal_to_noise_ratio");
    sensor_msgs::PointCloud2Iterator<float> iter_exProb(
        radarData,
        "existence_probability");

    // Convert to Cartesian coordinates
    for(size_t i = 0; i < radarData.width; ++i,
               ++iter_x,
               ++iter_y,
               ++iter_z,
               ++iter_azi,
        ...
        *iter_x = static_cast<float>(detection.RadialDistance *
                                     cos(detection.ElevationAngle) *
                                     cos(detection.AzimuthAngle));
        *iter_y = static_cast<float>(detection.RadialDistance *
                                     cos(detection.ElevationAngle) *
                                     sin(detection.AzimuthAngle));
        *iter_z = static_cast<float>(detection.RadialDistance *
                                     sin(detection.ElevationAngle));
...
void SensorBridgeNode::publishCameraData(
    std::unique_ptr<CameraDataDeserializerDefault>& deserializer,
    uint8_t sensorId,
    int cameraImageType) {
    // Set camera image type
    CameraImageType requestedCameraImageType =
        static_cast<CameraImageType>(cameraImageType);
    ...
    sensorData.getSensorData(&requestedImage,
                             deserializer.get(),
                             requestedCameraImageType);

    // Print some data points
    if(this->verbosePrinting) {
        std::cout << "\tRequested camera sensor with ID " << unsigned(sensorId)
                  << " at maneuver time: "
                  << sensorData.getSensorHeader().m_uiTimeStamp << "\xE6s"
                  << std::endl;
        std::cout << "\t\twidth=" << requestedImage->Width
                  << " height=" << requestedImage->Height
                  << " row_pitch=" << requestedImage->RowPitch
                  << " stride=" << requestedImage->Stride << std::endl;
    }
    ...
    cameraData.data.assign(requestedImage->Data,
                           requestedImage->Data +
                               (cameraData.height * cameraData.step));
