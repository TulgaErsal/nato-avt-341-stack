...
bool Costmap::HasData() const
{
	return std::any_of(layers_.begin(), layers_.end(),
		[](const std::shared_ptr<CostmapLayer>& layer) { return layer->HasData(); }
	);
}
...
void Costmap::UpdateSlopeParameters(optional<float> tr, optional<float> tr_max) {
	thresholds_.Update(tr, tr_max);
	node_ref_->log_info(
		"Updating grid dilation with slope parameters: thresh=%.2f, thresh_max=%.2f",
		thresholds_.thresh,
		thresholds_.thresh_max
		);
	for (const auto& layer : layers_) {
		layer->RecomputeGridDilation();
	}
}
//
...
avt_341::msg::OccupancyGridUpdate Costmap::GetGridUpdate(bool is_segmentation) {
...
avt_341::msg::OccupancyGrid Costmap::GetGrid(bool is_segmentation) {
...
avt_341::msg::OccupancyGrid Costmap::GetGrid(double x, double y, double width, double height, bool is_segmentation) {
