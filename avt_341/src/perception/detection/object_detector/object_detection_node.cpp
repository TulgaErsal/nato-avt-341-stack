...
void ObjectDetectorNode::Initialize() {
    try {
        ...
        RCLCPP_ERROR(
            get_logger(),
            "Package not found - is your environment sourced correctly?");
        rclcpp::shutdown();
    }
    ...
    if(!external_model_path_.empty()) {
        ...
        model_path = package_prefix_ + "/share/avt_341/data/detection/models/" +
            model_name_;
    }
    ...
    if(names_stream.fail()) {
        throw rclcpp::exceptions::InvalidParameterValueException(
            "Names file is not inaccessible or not existing.");
    }
    ...
    for(size_t i = 0; i < classes_.size(); ++i) {
        classes += " <[" + std::to_string(i) + "] " + classes_[i] + "> ";
    }
    RCLCPP_DEBUG(get_logger(),
                 "Loaded %i object detection model classes > %s",
                 int(classes_.size()),
                 classes.c_str());
...
void ObjectDetectorNode::CreatePublishers() {
    ...
    bounding_boxes_publisher_ =
        create_publisher<vision_msgs::msg::BoundingBox2D>(
            "detections/bounding_boxes",
            1);

    if(use_visualizer_) {
        detections_image_publisher_ =
            create_publisher<sensor_msgs::msg::Image>("detections/image/raw",
                                                      1);
    } else {
        RCLCPP_DEBUG(get_logger(),
...
void ObjectDetectorNode::PublishInfoMessages() {
    ...
    RCLCPP_DEBUG(get_logger(),
                 "Publishing vision_msgs/msg/VisionInfo message ...");
    ...
    RCLCPP_DEBUG(get_logger(),
                 "Publishing vision_msgs/msg/LabelInfo message ...");
    ...
    for(size_t i = 0; i < classes_.size(); ++i) {
        auto vision_class_message =
            std::make_shared<vision_msgs::msg::VisionClass>();
...
void ObjectDetectorNode::ImageCallback(
...
void ObjectDetectorNode::DetectionCallback() {
    ...
    if(use_visualizer_) {
        visualization_thread_ =
            std::thread(&ObjectDetectorNode::PublishDetectionImage,
                        this,
    ...
    if(detection_rate_ > 0.0 &&
       mean_detection_time / 1.0e3 > 1.0 / detection_rate_) {
...
void ObjectDetectorNode::PublishDetectionImage(
    cv::Mat image,
...
double ObjectDetectorNode::GetMeanDetectionTime(double last_detection_time) {
    ...
    for(boost::circular_buffer<double>::const_iterator buffer_iterator =
            time_buffer_.begin();
