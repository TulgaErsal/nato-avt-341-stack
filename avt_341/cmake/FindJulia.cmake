...
#
# @brief Provide platform-independent CMake FindPackage functionality for the
#        Julia C API. The script is based on the one provided in the GitHub
#        repository barche/embedding-julia, distributed under MIT license.
#
# @date 03/11/2023
#
...
# Report the Julia version to the CMake log.
...
if(DEFINED ENV{JULIA_INCLUDE_DIRS})
    ...
    execute_process(COMMAND ${Julia_EXECUTABLE} --startup-file=no -E "julia_include_dir = joinpath(match(r\"(.*)(bin)\",Sys.BINDIR).captures[1],\"include\",\"julia\")\n
...
# Julia libraries
# ---------------
execute_process(COMMAND ${Julia_EXECUTABLE} --startup-file=no -E "using Libdl; abspath(dirname(Libdl.dlpath(\"libjulia\")))"
...
# Define the _LIBRARY_DIR CMake variable.
set(Julia_LIBRARY_DIR "${Julia_LIBRARY_DIR}"
    CACHE
    PATH "Julia library directory")

# Find library on Windows.
if(WIN32)
    ...
    # Find library on all other platforms.
    find_library(Julia_LIBRARY
...
# Report the libraries and their containing folders to the CMake log.
...
# Check for Julia headers
find_path(Julia_MAIN_HEADER julia.h HINTS ${Julia_INCLUDE_DIRS})

# Report the LLVM compiler version to the CMake log.
MESSAGE(STATUS "Julia_LLVM_VERSION: ${Julia_LLVM_VERSION}")
# ------------------



# FindPackage() functionality
# ---------------------------
include(FindPackageHandleStandardArgs)

# Define standard arguments for FindPackage()
find_package_handle_standard_args(Julia
    REQUIRED_VARS
      Julia_LIBRARY Julia_LIBRARY_DIR Julia_INCLUDE_DIRS Julia_MAIN_HEADER
