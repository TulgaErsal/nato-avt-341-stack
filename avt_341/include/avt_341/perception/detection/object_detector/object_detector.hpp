...
class ObjectDetector {
  public:
    ...
    /**
     * @brief Read object classes from a plain-text file with one class
     * per row into a vector of strings.
     *
     * @param file
    ...
    /**
     * @brief Load a TorchScript model and class names files from a specified
     * path.
     *
     * @param path Path to an object detector model (including the model
     * filename, excluding the extension).
     */
    void Load(std::string path);

    /**
     * @brief Warm up the neural network by performing inference with an empty
     * tensor.
     */
    void Warmup();
    ...
    /**
     * @brief Set the Intersection-Over-Union metric threshold during
    ...
    /**
     * @brief Set the maximum number of detections in a single frame detection
     * task.
     *
  ...
  private:
     ...
     * @return std::vector<std::string> Vector of split substrings (excluding
     * the delimiter).
     */
    std::vector<std::string> SplitByDelimiter(const std::string& string,
     ...
     */
    void GetLetterbox(cv::Mat& image,
                      cv::Mat& letterboxed_image,
                      cv::Size size,
                      cv::Scalar value = cv::Scalar(139.0));

    /**
     * @brief Convert a tensor of XYWH bounding box coordinates to a tensor of
     * XYXY bounding box coordinates.
     *
     * @param xywh_tensor Input tensor of bounding boxes XYWH coordinates.
     * @return torch::Tensor Output tensor of bounding boxes in XYXY
     * coordinates.
     */
    torch::Tensor FromXYWH2XYXY(const torch::Tensor& xywh_tensor);

    /**
     * @brief Scale a tensor of bounding box coordinates referenced to an input
     * image shape to an output image shape.
     *
     ...
     * @return torch::Tensor
     */
    torch::Tensor ScaleBoundingBoxes(torch::Tensor& bounding_boxes,
     ...
     */
    torch::Tensor
    NonMaximumSuppressionKernel(const torch::Tensor& bounding_boxes,
                                const torch::Tensor& scores);

    /**
     * @brief Run a YOLOv8 Torch tensor through the Non-Maximum Suppression
     * (NMS) kernel.
     *
