...
def generate_launch_description():
    ...
    base_launch = launch.actions.IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(avt_341_dir, 'launch', 'mission_base.launch.py')),
        launch_arguments={'waypoints_file': waypoints_file,
                          'robot_description': robot_desc,
                          'use_sim_time': 'False',
                          'goal_dist': '5.0',
                          'path_look_ahead': '30.0',
                          'shutdown_behavior': '1'}.items()
    )
    ...
    mavs_vehicle = launch_ros.actions.Node(
        ...
        remappings=[
                (('/mavs/cmd_vel'), '/avt_341/cmd_vel'),
                (('/mavs/odometry_true'), '/avt_341/odometry')
            ],
        output='screen',
        emulate_tty=True
    )
