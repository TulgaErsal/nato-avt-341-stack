...
class Trajectory:
    ...
    def state_prediction(self,timestamp):
        ...
        # not using linearisation of nonlinear motion because (E)KF
        if omegaT == 0:
            # avoid divide by zero omegaT = 0 means continue in current direction
            # this is basically what linearisation predicts also for omegaT not zero
            current_predicted_state[Trajectory.X_ID_X,0] = previous_state[Trajectory.X_ID_X,0] + v_fwdT * np.cos(theta)
            current_predicted_state[Trajectory.X_ID_Y,0] = previous_state[Trajectory.X_ID_Y,0] + v_fwdT * np.sin(theta)
        else:
            dx = 2*v_fwdT/omegaT*np.sin(omegaT/2)
            current_predicted_state[Trajectory.X_ID_X,0] = previous_state[Trajectory.X_ID_X,0] + dx * np.cos(theta+omegaT/2)
            current_predicted_state[Trajectory.X_ID_Y,0] = previous_state[Trajectory.X_ID_Y,0] + dx * np.sin(theta+omegaT/2)                
	
        current_predicted_state[Trajectory.X_ID_YAW,0] = (current_predicted_state[Trajectory.X_ID_YAW,0]+ omegaT +np.pi) % (np.pi*2) - np.pi
